package com.kakaxi.androidmvp.network.uploaddown;


import com.kakaxi.androidmvp.network.LoggerInterceptor;
...
public class ServiceGenerator {
    ...
    public static <T> T createDownloadService(Class<T> tClass, DownloadProgressListener listener){
        OkHttpClient client = HttpClientHelper.addProgressDownloadListener(new OkHttpClient.Builder(),listener)
                .addInterceptor(new LoggerInterceptor("fightdemo", HttpConfig.DEBUG)).build();
        return builder
                .client(client)
                .build()
                .create(tClass);
